...
const WORDS = ["weird", "i", "want", "dead", "Mitsubishi", "outlander", "please firepup stop killing me", "please 9pfs stop", "platypus", "pineapple", "orange", "apple", "devarsh", "firepuppy", "anirudh", "9pfs", "h", "hello-smile6", "is", "cool", "argentina", "antarctica", "usa", "guns", "boom", "lol", "why", "confusion", "neoroll", "wooo", "amc", "irc"]
  ...
  fn: (ev) => {
    ...
    if(ev.message.startsWith("squawk ")) {
      ...
      if(lastSquawk.length < 1) {
        ...
        ev.reply(template("Squawk", ev.nick, `You won in ${((Date.now() - lastSquawkTime) / 1000).toFixed(3)} seconds!`));
        lastSquawk = "";
      } else {
