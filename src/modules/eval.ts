...
export default {
  fn: async (ev) => {
    if(ev.message.startsWith("eval ")) {
      console.log(ev.nick, "used the eval function!")
      ...
      if(!tioLangs.includes(language)) {
        ev.reply(template("Eval", ev.nick, `The language ${language} is not supported by TIO.`));
        return;
      }
      if(evalString.length > 120) {
        ev.reply(template("Eval", ev.nick, `Your code is ${evalString.length - 120} character(s) over the limit.`));
        return;
      }
      ...
      if(req.status === "timed out") {
        ev.reply(template("Eval", ev.nick, `Your code timed out.`))
        return
      }
      ev.reply(template("Eval", ev.nick, `Output: ${req.output.replace(/\n/gm, "\\n")}`))
    }
  }
} satisfies Module;
